...
window.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        console.log('기본 건강 정보 없음');
    }
    
    // 건강 데이터 로드
    await loadHealthData(userId, healthInfo);
});

// 서버에서 건강 데이터 불러오기
async function loadHealthData(userId, healthInfo) {
    try {
        const response = await fetch(`${BASE_URL}/health/records/user/${userId}`, {
            method: "GET",
        ...
        if (!data || data.length === 0) {
            ...
            // 차트 데이터 업데이트
            updateChartData(data, healthInfo);
            
            // 최신 값으로 카드 업데이트
            updateCurrentValues(data[data.length - 1], healthInfo);
...
// 서버 데이터를 차트 형식으로 변환
function updateChartData(healthData, healthInfo) {
    ...
    // BMI 데이터 계산 (healthInfo에서 키 정보 사용)
    ...
    if (healthInfo && healthInfo.height_cm) {
        ...
        chartData['BMI'].data = recentData.map(() => 0);
    }
    
    // 위험도 데이터 (클라이언트에서 계산)
    chartData['위험도'].labels = labels;
    chartData['위험도'].data = recentData.map(item => {
        return calculateRiskScore(item, healthInfo);
    });
}
...
// 현재 값 카드 업데이트
function updateCurrentValues(latestData, healthInfo) {
    // 혈압
    ...
    // 위험도
    const riskScore = calculateRiskScore(latestData, healthInfo);
