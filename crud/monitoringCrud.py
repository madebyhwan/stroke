...
# 요청 삭제
async def delete_request_by_id(db: AsyncIOMotorDatabase, request_id: str) -> bool:
    """모니터링 요청 삭제"""
    result = await db.monitoring_requests.delete_one({"_id": request_id})
    return result.deleted_count > 0

# 특정 요청 존재 확인
async def request_exists(db: AsyncIOMotorDatabase, patient_id: str, requester_id: str) -> bool:
    """환자-모니터 간 관계가 이미 존재하는지 확인"""
    relation = await db.monitoring_requests.find_one({
        "patient_id": patient_id,
        "requester_id": requester_id,
        "status": MonitoringStatus.PENDING
    })
